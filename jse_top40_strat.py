...
def sharpe_ratio(daily_returns: pd.Series, rf: float = 0.0) -> float:
    ...
    ann_vol = dr.std(ddof=0) * math.sqrt(TRADING_DAYS)
    er = ann_ret - rf
    return float(er / ann_vol) if ann_vol > 0 else 0.0
...
def max_drawdown(equity: pd.Series) -> Tuple[float, int]:
    ...
    dd = eq / peaks - 1.0
    max_dd = float(dd.min())
    end_idx = dd.idxmin() if not dd.empty else None
    if end_idx is None:
        return 0.0, 0
    start_idx = (eq.loc[:end_idx]).idxmax()
    dur = (end_idx - start_idx).days if hasattr(end_idx, "to_pydatetime") else 0
    return max_dd, int(dur)

def equity_curve(returns: pd.Series) -> pd.Series:
    return (1 + returns.fillna(0.0)).cumprod()

def apply_turnover_costs(daily_returns: pd.Series, positions: pd.Series, cost_bps: float) -> pd.Series:
...
def explain_sharpe(v: float) -> str:
    if v < 0:   return "This strategy loses value relative to its risk. Consider avoiding."
    if v < 0.5: return "Risk-adjusted return is weak and may be unstable."
    if v < 1.0: return "Modest risk-adjusted return. Consider smaller position sizes."
    if v < 1.5: return "Good risk-adjusted return. A reasonable candidate."
    if v < 2.0: return "Very good risk-adjusted return."
    return "Exceptional risk-adjusted return — double-check it’s not overfit."

def explain_cagr(v: float) -> str:
    if v < 0:    return "Loses money over the period."
    if v < 0.05: return "Low growth. Might not beat passive alternatives."
    if v < 0.15: return "Moderate growth."
    return "High growth. Ensure the risks are acceptable."

def explain_maxdd(v: float) -> str:
    if v > -0.10: return "Shallow worst loss — easier to live with."
    if v > -0.25: return "Manageable worst loss for many investors."
    if v > -0.40: return "Deep loss — expect uncomfortable periods."
    return "Very deep loss — only for high risk tolerance."

def explain_dd_days(d: int) -> str:
    if d < 200:   return "Recoveries are relatively quick."
    if d < 500:   return "Recoveries take a while; patience required."
    if d < 1000:  return "Recoveries are slow; long patience needed."
...
def build_summary_block(name: str, c: float, s: float, dd: float, ddd: int) -> str:
    ...
    lines.append(f"- CAGR (growth rate): {pct(c)}. {explain_cagr(c)}")
...
if run_btn:
    ...
    summary_df = pd.DataFrame(rows, columns=["Strategy","CAGR","Sharpe","MaxDD","DD_Days"]).set_index("Strategy")
    ...
    if REPORTLAB_AVAILABLE:
        ...
        story.append(Paragraph(f"Costs: fees {fees_bps} bps, slippage {slippage_bps} bps (total {total_bps} bps per turnover)", styles["Normal"]))
        story.append(Spacer(1, 0.4*cm))
        ...
        doc.build(story)
        st.download_button("Download PDF Report", data=pdf_buf.getvalue(), file_name="strategy_report.pdf", mime="application/pdf")
    else:
